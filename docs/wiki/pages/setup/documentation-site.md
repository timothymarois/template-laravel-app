...
|---|---|
| Build command | `pip install "git+https://github.com/timothymarois/wiki-builder@v0.3.2" && wiki check && wiki publish _site`[^build] |
...
[^build]: `wrangler.jsonc` — the comment recording the build command; `scripts/dev-wiki.sh` —
    `WIKI_VERSION=v0.3.2`; Cloudflare Docs —
...
[^check]: `package.json` — `check:wiki` runs `./scripts/dev-wiki.sh check`; `.github/workflows/wiki.yml` —
    the `Wiki check` job runs `timothymarois/wiki-builder@v0.3.2` on every pull request and a push to `main`.
