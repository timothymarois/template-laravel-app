...
**When to use:** You are about to create a branch, or write a commit message. Read it before the first
`git checkout -b` — a name is cheapest to get right before it exists.
**Prerequisites:** None.
...
The same set governs branches, commits, and pull-request titles:
...
Pick by what the change *does* for a reader of the history, not by which files it touched: a `refactor`
that fixes a bug is a `fix`.

## No machine-authorship branding
...
that produced a change tells a reviewer nothing they can act on, and dates the history permanently: the
diff is what is being judged, and it reads the same either way.
