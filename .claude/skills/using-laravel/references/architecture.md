...
Do this when the operation has multiple callers, coordinated steps, meaningful side effects, or an
independent test boundary. A small CRUD action can remain in its controller; do not create an action
class only to satisfy a layer count.
...
Prefer constructor or method injection so dependencies are visible. Avoid `app()` or `resolve()`
deep inside domain classes merely to avoid declaring a dependency.
...
## Define a Deterministic Product Sort

Without an explicit `ORDER BY`, row order is undefined. Choose a deterministic default from the
product requirement; newest-first is common, not universal.
...
Example when the product requires newest-first:
```php
...
Use jobs when the work must survive process crashes or needs retry logic. Use `defer()` only for
lightweight post-response work that may be lost if the PHP process exits.
...
Do not use Context as an invisible replacement for an explicit argument when correctness depends on
the value. Never place credentials or secrets in visible log context.
...
Measure the process/driver overhead and do not parallelize operations that share a transaction, must
run in order, or depend on each other's results. Verify `defer()`, Context, and Concurrency against
the installed Laravel version before using them.

## Keep Jobs as Transport When Reuse Matters

Incorrect — the operation is reachable only through a worker:
...
final class ImportPriceListJob implements ShouldQueue
{
    ...
    {
        // Entire import workflow lives here.
    }
}
```

Correct — the job owns queue policy and delegates the reusable operation:
...
Keep required business work visible at the operation that performs the write. Observers can own
narrow model-level behavior, but mass updates do not dispatch model events.
