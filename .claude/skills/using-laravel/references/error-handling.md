...
## Keep Fallbacks Observable

Incorrect — the product degrades silently and operations cannot see the dependency failure:

```php
try {
    return $catalog->recommendations($user);
} catch (Throwable) {
    return collect();
}
```

Correct — report unexpected failure or emit the application's deliberate dependency metric before
returning an accepted fallback:
...
Use `ShouldntReport`, `dontReport()`, duplicate suppression, and throttling only after preserving
another response, metric, or sample that makes the condition observable. Attach safe identifiers to
exception context; never log credentials, tokens, or raw personal data.
