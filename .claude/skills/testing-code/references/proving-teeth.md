...
it('calculates the total', function () {
    $calculator = Mockery::mock(DiscountCalculator::class);
    $calculator->shouldReceive('discount')->andReturn(5);

    expect($calculator->discount($order))->toBe(5);
});

// Good: fails the moment the real rule changes.
it('applies the discount to orders over the threshold', function () {
    expect(discountFor(new Order(total: 105)))->toBe(5);
});
```
