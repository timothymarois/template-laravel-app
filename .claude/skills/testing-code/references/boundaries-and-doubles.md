...
$repo = Mockery::mock(RateRepository::class);
$repo->shouldReceive('loadRates')->once();          // breaks on any harmless refactor
(new PriceService($repo))->quote($order);

// Good: state verification. Fails when the behavior changes, survives restructuring.
$rates = new StubRates(['GBP' => '1.00']);
expect((new PriceService($rates))->quote($order))->toBe('42.00');

// Also good, when the interaction IS the requirement.
$payments->shouldHaveReceived('charge')->with($order->id, '42.00');
```
