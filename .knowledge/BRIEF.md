...
- **Active areas:** the Laravel app itself — auth, the admin dashboard and user management, the
  Vue/Inertia UI and shadcn-vue component showcase, health checks, and testing. Multi-tenancy ships but
  is inert by default.
- **Out of scope:** product-specific features (each fork owns those), and any cross-fork bookkeeping
  (which fork runs which template version).
