# Changelog — Legacy (pre-v4)

Archived history for `template-laravel-app` releases **before v4.0.0**. Kept for reference only — these predate the current architecture (shadcn-vue, the `migrations/` guides, and the [changelog rules](README.md#changelog-rules)). They are **frozen**: don't reformat or extend them. Current history lives in [`CHANGELOG.md`](CHANGELOG.md).

These entries reference the old `atlas-ui` package and PrimeVue eras, before the template was rebuilt on shadcn-vue in v4.0.0.
